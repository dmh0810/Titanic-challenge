...
#Confusion Matrix rf
confusionMatrix(pred_rf, titanic_test$survived)
...
svm_pred_prob <- predict(svm_modell, test2, type = "prob")
...
preds_list <- list(logit_pred, pred_rf_prob[,1], pred_gbm, knn_pred_prob[,1], nb_pred_prob[,1], svm_pred_prob[,1])
m <- length(preds_list)
actuals_list <- rep(list(titanic_test$survived),m)
